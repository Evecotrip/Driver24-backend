 ...
 */
export const registerGuestDriver = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (files?.dlImage?.[0]) {
            const dlResult = await uploadImage(
                ...
                'dl-images',
                `${Date.now()}-dl-${email.replace(/[^a-zA-Z0-9]/g, '-')}`
            );
        ...
        if (files?.panImage?.[0]) {
            const panResult = await uploadImage(
                ...
                'pan-images',
                `${Date.now()}-pan-${email.replace(/[^a-zA-Z0-9]/g, '-')}`
            );
        ...
        if (files?.aadharImage?.[0]) {
            const aadharResult = await uploadImage(
                ...
                'aadhar-images',
                `${Date.now()}-aadhar-${email.replace(/[^a-zA-Z0-9]/g, '-')}`
            );
 ...
 */
export const cleanupExpiredPendingDrivers = async (): Promise<number> => {
    try {
